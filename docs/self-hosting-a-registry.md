...
```bash
capt marketplace add https://example.com/open-design-marketplace.json --trust restricted
capt marketplace refresh <id>
capt marketplace search "deck" --json
```
...
```bash
capt marketplace login https://github.example.com/org/plugin-registry
capt marketplace add https://raw.github.example.com/org/plugin-registry/main/open-design-marketplace.json --trust trusted
```
...
The CLI vocabulary stays the same: `capt marketplace add/search/doctor`,
`capt plugin install/upgrade/publish/yank`.
