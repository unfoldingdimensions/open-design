...
```bash
capt plugin scaffold --id figma-workflow --title "Figma workflow" --out ./plugins/community
```
...
separated registry paths are used by catalogs, not by `capt plugin scaffold`.
The generated `open-design.json` is the OpenDesign sidecar next to `SKILL.md`.

## 2. Validate And Pack

```bash
capt plugin validate ./plugins/community/figma-workflow --no-daemon
capt plugin pack ./plugins/community/figma-workflow
```
...
`capt plugin pack` writes the archive next to the plugin folder by default.
...
```bash
capt plugin publish figma-workflow --to open-design --repo https://github.com/acme/figma-workflow
```
...
capt plugin install figma-workflow
capt plugin info figma-workflow --json
```
...
```bash
capt plugin yank figma-workflow@1.0.0 --reason "Security issue"
```
