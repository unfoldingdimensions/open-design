...
| **Call — `api`** | the tool's own declared `/api/*` route + `capt` subcommand (`ApiToolDescriptor.api` / `.cli`) — no round-trip envelope | `tools/call` (api surface) |
...
carries both its `/api/*` route and its `capt` subcommand, so the UI/CLI dual-track
...
invokes its declared route directly (and it is reachable as an `capt` subcommand),
...
> A tool may be `surface: 'browser'` — and ship without an `capt` CLI form — ONLY
...
> capability: `surface: 'api'` with both an `/api/*` route and an `capt` subcommand.
