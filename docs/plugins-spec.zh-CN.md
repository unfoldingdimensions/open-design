...
| Headless CLI / code agent | Claude Code、Cursor、Codex、CI 或脚本调用 `capt` | `capt run start --project p_abc --plugin make-a-deck --inputs '{"audience":"investors","topic":"AI design ops"}'` | 不打开 desktop，也得到同一套 manifest resolution、context chips 和 run events。 |
...
可以同时存在多个 marketplaces。用户通过 `capt marketplace add <url>` 注册额外 index（Vercel 的、OpenClaw 的 clawhub、企业私有 catalog）。默认情况下，用户添加的 marketplace 只是 discovery source，它里面的插件仍然以 `restricted` 安装；只有官方内置 marketplace 或用户显式执行 `capt marketplace add <url> --trust trusted` / `capt marketplace trust <id> --trust trusted` 后，来自该 marketplace 的插件才可以默认继承 `trusted`。
...
冲突解决：按 normalized `name` / plugin id；数字越小优先级越高。legacy `SKILL.md` location 被 adapter 合成为 plugin record，但不会被复制到 `<daemonDataDir>/plugins/`，除非用户显式执行 `capt plugin install`。这样现有 Claude skills 继续零配置可用，新 plugin bundle 也有清晰的安装根。
...
- 插件可能在两次 run 之间被 `capt plugin upgrade <id>` 升级。
...
3. **Replay / export**：`capt plugin replay <runId> --snapshot-id <snapshotId>` 发出不可变 snapshot 与 rerun bundle；`capt plugin export --snapshot-id <snapshotId> --as <target> --out <dir>` 则从该 snapshot 而非 live manifest 解析 export。调用方显式重新 apply 并 start replay run，从而保证 plugin 升级后旧 run 仍可复现。
...
`AppliedPluginSnapshot` 只在 daemon side 写入；CLI / UI 客户端只读。Plugin 升级或 marketplace ref 漂移触发 `capt plugin doctor` 把受影响的历史 snapshot 标为 `stale`，但**永不重写**：reproducibility 优先于 freshness。
...
4. **不响应 surface ≠ 阻塞 run。** 每个 surface 必须声明 `timeout`（默认 5 分钟）与 `onTimeout`（`abort` / `default` / `skip`）；CLI 在 ND-JSON 流中以 `genui_surface_request` event 暴露同一份 surface 描述，使 headless 自动化可以 `capt ui respond <runId> <surface-id> --value-json …` 在另一个进程中作答，或在不需要时优雅 skip（详见 §10.3）。
...
| `media-image` / `media-video` / `media-audio` | `capt media generate` | 带 provider config 的媒体生成 | new-generation, tune-collab |
...
- `until` 是 daemon 解析的轻量 expression（仅支持比较运算与已知信号变量：`critique.score`、`iterations`、`user.confirmed`、`preview.ok`），不是任意 JS。`capt plugin doctor` 会校验语法。
...
4. 用户通过 `capt run respond <runId> --json '{"action":"break-loop"}'` 或 UI 中的「Stop refining」按钮可以随时打断。

Devloop 的两条硬约束：

- **必须有 `until`**：`repeat: true` 但缺 `until` 的 pipeline 在 `capt plugin doctor` 中报错；daemon 拒绝执行。
...
`capt plugin doctor` 在 install / apply 阶段强制：(1) `oauth.route === 'connector'` 时，`oauth.connectorId` 必须出现在同 plugin 的 `od.connectors.required[]` 或 `od.connectors.optional[]` 中；(2) `oauth.route === 'mcp'` 时，`oauth.mcpServerId` 必须匹配 plugin 自带的某个 MCP server name。
...
5. **用户主动撤销：** UI / CLI 提供 `capt ui revoke <surface-id>`，把 row 翻到 `invalidated`。常用于 OAuth token 退出。

这条规则正面对接用户提出的需求："**同一个 project 用户授过权、做过确认，下一次（多轮 / 多会话）不要再问。**"

#### 10.3.4 Headless / CLI 行为

`capt run watch` / `capt run start --follow` 的 ND-JSON 流中包含 `genui_surface_request` event。第三方 code agent 有三种应对方式：
...
capt ui respond <runId> <surface-id> --skip          # 触发 onTimeout='skip'
capt ui revoke  <projectId> <surface-id>             # 跨 conversation 撤销
```
...
- `validate.ts`：JSON Schema（同时驱动 runtime checks 和 `capt plugin doctor`）。
...
| 新 `apps/daemon/src/plugins/snapshots.ts` | §8.2.1 不可变 snapshot 写入与读取；`status='stale'` 标记由 `capt plugin doctor` 触发；提供 replay helper 给 `POST /api/runs/:runId/replay`。 |
...
Migration 仅新增；未触碰现有 `projects` / `runs` / `conversations` 列语义。daemon 在 install / apply / run start / stage 完成时按上面 schema 写入。`capt plugin doctor` 在插件升级后比较 `manifest_source_digest` 把过期 snapshot 的 `status` 翻到 `stale`，但**永不删除或重写** snapshot 行——历史 run 的可复现性优先于存储成本。
...
| Method | Path | Purpose |
| --- | --- | --- |
| GET | `/api/applied-plugins/:snapshotId` | 读取一份不可变 snapshot；用于 audit、replay、`capt plugin export` |
...
> **Transport equivalence（经验规则）。** 上面每个 endpoint 都同时暴露为 CLI subcommand，并在适合 MCP 语义的地方暴露为 MCP tool。代码 agent 应该使用 CLI；只有 desktop web app 和 `capt …` 自己直接使用 HTTP。完整 CLI 表面见 §12。
...
- 每次 `capt plugin export` / `capt files upload --to <target>` / `capt deploy ...` 追加一行 `exportTargets` / `deployTargets`，**不**修改 `sourcePluginSnapshotId`。
...
| Mode | 运行内容 | 适用场景 | 入口 |
| --- | --- | --- | --- |
| **Headless** | 仅 daemon process；无 web bundle、无 electron | CI、server、container、Claude-Code-driven flows | `capt daemon start --headless`（Phase 2 新 flag） |
| **Web** | Daemon + local web UI（无 electron） | 纯浏览器部署、Linux 无 GUI dependencies | `capt daemon start --serve-web`（Phase 2 新 flag） |
| **Desktop** | Daemon + web bundle + electron shell | 完整产品体验（当前默认） | `capt`（当前默认，不变） |
...
- **Marketplace browsing**：UI 是 grid + filters + previews。CLI 是 `capt plugin list/info`、`capt marketplace search`、`capt plugin info <id> --json`，返回 UI 渲染的同一份 manifest。
- **Plugin apply**：UI 是点击卡片、chips 与 inputs 原地 hydrate。CLI 是 `capt plugin apply <id> --json`，返回完全相同的 `ApplyResult`。
- **Run streaming**：UI 是 chat bubbles、todo list、progress chrome。CLI 是 `capt run start --follow`，从 UI 消费的同一 `PersistedAgentEvent` discriminated union 发出 ND-JSON events。
- **Direction picker / question form**：UI 渲染为 inline cards。CLI 发出结构化事件到 stdout；agent 或 scripted wrapper 通过 stdin 或 `capt run respond <runId> --json '{...}'` 选择选项。
- **Live artifact preview**：UI 是 hot-reloading iframe。CLI 是 `capt files watch <projectId> --path <relpath>`，stream change events；用户用自己的工具打开文件。
...
| MCP stdio | MCP-aware agents（Claude Code、Cursor 等） | `capt mcp` 与 `capt mcp live-artifacts`（现有） |

当新能力发布时，CLI subcommand 是 primary contract。HTTP route 用于支撑 CLI；MCP server 把一组精选 CLI subcommands 暴露为 tools。版本策略：subcommand names、argument names、`--json` schemas 由 `packages/contracts` 管理并在 CI 中测试；breaking changes 跟随 `capt` bin 的 major-version bump。
...
`capt daemon start --headless` 是 §11.7 中 headless mode 的入口（无 web bundle、无 electron）。`capt daemon start --serve-web` 增加本地 web UI 但不启用 electron。二者都沿用现有 tools-dev port conventions（[`OD_PORT`, `OD_WEB_PORT`](../AGENTS.md)）。
...
| --- | --- | --- | --- |
| 64 | Daemon not running | `capt status`，然后启动 daemon | `{ host, port }` |
| 65 | Plugin not found / not installed | `capt plugin list` 后 `capt plugin install <source>` | `{ pluginId, candidateSources[] }` |
| 66 | Plugin restricted, capability required | `capt plugin trust <id> --capabilities …` 或重试时加 `--grant-caps …` | `{ pluginId, pluginVersion, required[], granted[], remediation[] }` |
...
| 70 | Provider not configured | 为 provider key 执行 `capt config set ...` | `{ provider, requiredKeys[] }` |
| 71 | Plugin requires daemon mode | 启动 daemon 或切换到 desktop / headless | `{ pluginId, mode: 'api-fallback' }`（§11.8） |
| 72 | Applied plugin snapshot stale | `capt plugin replay <runId> --snapshot-id <snapshotId>` 或 `capt plugin upgrade <id>` 后重新 apply | `{ snapshotId, pluginId, currentVersion, snapshotVersion }` |
| 73 | GenUI surface awaiting response | `capt ui show <runId> <surface-id>` 后用 `capt ui respond` 作答；或在调用前 `capt ui prefill` | `{ runId, surfaceId, kind, schema, prompt, persist, timeoutAt }`（§10.3） |
...
上面每个 group 都是对 [`apps/daemon/src/cli.ts`](../apps/daemon/src/cli.ts) 的增量。当前默认 `capt`（启动 daemon + 打开 web UI）保持不变。现有 `capt media`、`capt research`、`capt tools`、`capt mcp` 命令保持当前 contract。新增 groups 是对 `apps/daemon/src/server.ts` 中既有 desktop UI endpoints 的包装（project create/list、run start/watch、file upload/list/read），再加上 §11.5 的新插件/marketplace/atoms endpoints。
...
任何能通过 desktop UI 设置的内容，也都能通过 `docker exec od capt config set ...` 设置。本文件绝不能提供具体存储路径。
...
- 推荐默认 `OD_TRUST_DEFAULT=restricted`。能力（`mcp`、`subprocess`、`bash`、`network`）需要 operator 通过 `capt plugin trust <id>` 显式 opt-in，或通过 `OD_TRUSTED_PLUGINS` allow-list env var 授权。
...
- **Plugin CLI verbs：** `capt plugin install/list/info/uninstall/apply/doctor`。`capt plugin apply --json` 是 Phase 2 inline rail 与外部 code agents 的必要能力，必须在此返回包含 `appliedPlugin: AppliedPluginSnapshot` 的 `ApplyResult`。
- **Headless MVP CLI 闭环（新提前）：** `capt project create/list/info`、`capt run start/watch/cancel`（含 `--follow` 与 ND-JSON streaming）、`capt files list/read`。这些包装 desktop UI 今天已经使用的 endpoints（`POST /api/projects`、`POST /api/runs`、`GET /api/runs/:id/events`、project list/read endpoints），不新增 HTTP surface，只新增 CLI surface。
...
- **§10.3 GenUI 最小集落地：**
  ...
  - CLI：`capt ui list/show/respond/revoke/prefill`；`capt run watch` ND-JSON 包含 `genui_*` event；exit code 73 接通。
...
- **§9 connector capability gate 落地：**
  ...
  - `capt plugin doctor`：(a) `od.connectors.required[].id` 必须存在于 `connectorService` catalog；(b) `tools[]` 是该 connector `allowedToolNames` 子集；(c) `oauth.route='connector'` 时 `oauth.connectorId` 必须出现在同 plugin 的 connector 声明里。
  - exit code 66 / `409 capabilities-required` 的 `data.required` 含 `connector:<id>` 形态。
  - `capt plugin trust` / `capt plugin apply --grant-caps` 接受 `connector:<id>` 形态（§9.1）。
...
(f) **Connector trust gate**：local plugin 声明 `od.connectors.required = [{ id: 'slack', tools: ['channels.list'] }]`，**不**显式 grant `connector:slack`：
  ...
  - apply（无 `--grant-caps`） → exit 66 / 409，`data.required` 含 `connector:slack`，`data.remediation` 含 `capt plugin trust <id> --capabilities connector:slack`。
...
Validation：扩展 §12.5 的 walkthrough：`capt project import` 一个本地文件夹 → `capt plugin replay <runId> --snapshot-id <snapshotId>` 取回不可变 rerun bundle → 在 import 出来的 project 上执行 `capt plugin apply` + `capt run start`。

### Phase 3 — Federated marketplace + tiered trust（3–5 天）

- `capt marketplace add/remove/trust/list/refresh`；用 `capt marketplace trust <id> --trust restricted` 降低 trust tier。`capt plugin install <name>` 通过 marketplaces 解析。
...
- `capt plugin doctor <id>` 执行完整 validation。
...
- `capt plugin export <projectId> --as od|claude-plugin|agent-skill`：从已有 project 生成 publish-ready folder。
- `capt plugin run <id> --inputs <json> --follow`：apply + run start + watch 的 shorthand wrapper。
- `capt plugin scaffold` interactive starter。
- `capt plugin publish --to anthropics-skills|awesome-agent-skills|clawhub` 打开 PR template。
...
(b) 「UI vs CLI parity test」：挑 5 个 desktop-UI workflows，仅通过 `capt …` 重放，byte-for-byte 比较 produced artifacts（对应 §12.6 implementation rule）。
...
| `applied_plugin_snapshots` 表无界增长 | 按 PB2（已决）：未被引用的 snapshot 默认 30 天后过期；被 run / conversation / project 引用的 snapshot 永久 pin（reproducibility 优先）；GC worker 与 `capt plugin snapshots prune --before <ts>` 提供清理通道。 |
...
- **`capt plugin run` headless contract**：当前是否足够，还是也为非 CLI agents 暴露 HTTP POST endpoint？（默认：v1 只 CLI；Phase 4 如需要再补 HTTP。）
...
- **Discovery-time hot reload**：daemon 是否 watch `<daemonDataDir>/plugins/`（开发体验好），还是只在 `capt plugin install/upgrade/uninstall` 后 reload（稳定性高）？（默认：watch，500ms debounce。）
- **Versioning policy**：安装时 pin tag/SHA，还是默认跟踪 default branch 并提供 opt-in pin？（默认：安装时 pin resolved ref；`capt plugin upgrade <id>` 重新 resolve。）
...
- ~~**`AppliedPluginSnapshot` 留存策略**~~：**已按 PB2 解决。** 被 run / conversation / project 引用的 snapshot 永久 pin；未引用 snapshot 默认 30 天过期；`OD_SNAPSHOT_RETENTION_DAYS` 是 operator opt-in；`capt plugin snapshots prune` 保留为强制清理出口。
...
- `live-artifact` 的 live preview 与 `capt files watch`（§12）合在一起，让 hot reload 和 CLI co-watch 在 v1 都能跑通。
...
- **substrate**（底层基础）= v1 spec 给 plugin 作者的 primitive：manifest 字段、capability 词汇表、atom catalog、pipeline / devloop / GenUI / connector / MCP / `capt files` / `parentArtifactId` / `AppliedPluginSnapshot`（§5–§11.5.1）。
...
前置：用户先 `capt project import /path/to/old-repo`，project cwd 就是真实仓库。
...
| `trusted` | 一方 / 显式 trust 的 marketplace | 否 —— 安装即自动授权 | 是 —— `capt plugin upgrade <id>` 可以拉新版本 | `marketplace` |
...
3. `capt plugin uninstall` 拒绝 uninstall `bundled` plugin（会让 daemon 失能）；`capt plugin upgrade <id>` 会以 `409 bundled-plugin` 拒绝，因为 bundled plugin 只随 daemon image 升级替换。
