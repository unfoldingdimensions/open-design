# OpenDesign — Historical Product Spec
...
┌────────────▼──────────────────┐     ┌────────▼─────────────────┐
│   Local Daemon (capt daemon)   │     │   Anthropic Messages API │
...
- A third party can author a skill in a separate git repo, publish it, and have a user install it by running `capt skill add <git-url>` without touching OD's source.
