...
> OpenDesign постачається як **навички, CLI та сервер MCP**, які основні кодувальні агенти споживають нативно. Щойно OD встановлено, одна команда `capt mcp install <agent>` під'єднує сервер MCP до конфігурації цього агента, і ви викликаєте ті самі інструменти зсередини будь-якого агента.

| Кодувальний агент / платформа &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | Статус &nbsp;&nbsp; | Встановлення сервера MCP одним рядком &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|---|:---:|---|
| [Claude Code](https://docs.anthropic.com/en/docs/claude-code) | ✅ Підтримується | `capt mcp install claude` |
| [Codex CLI](https://github.com/openai/codex) | ✅ Підтримується | `capt mcp install codex` |
| [DeepSeek Reasonix](https://github.com/esengine/DeepSeek-Reasonix) | ✅ Підтримується | `capt mcp install reasonix` |
| [Raven](https://github.com/EverMind-AI/Raven) | ✅ Підтримується | `capt mcp install raven` |
| [Cursor](https://www.cursor.com/cli) | ✅ Підтримується | `capt mcp install cursor` |
| [VS Code + GitHub Copilot](https://github.com/features/copilot) | ✅ Підтримується | `capt mcp install copilot` |
| [GitHub Copilot CLI](https://github.com/features/copilot/cli) | ✅ Підтримується | `capt mcp install copilot` |
| [OpenCode](https://opencode.ai/) | ✅ Підтримується | `capt mcp install opencode` |
| [OpenClaw](https://github.com/openclaw/openclaw) | ✅ Підтримується | `capt mcp install openclaw` |
| [Antigravity](https://antigravity.google) | ✅ Підтримується | `capt mcp install antigravity` |
| [Cline](https://github.com/cline/cline) | ✅ Підтримується | `capt mcp install cline` |
| [Trae](https://www.trae.ai/) | ✅ Підтримується | `capt mcp install trae` |
| [Kimi CLI](https://github.com/MoonshotAI/kimi-cli) | ✅ Підтримується | `capt mcp install kimi` |
| [Kiro](https://kiro.dev) | ✅ Підтримується | `capt mcp install kiro` |
| [Pi Agent](https://github.com/badlogic/pi-mono) | ✅ Підтримується | `capt mcp install pi` |
| [Mistral Vibe CLI](https://github.com/mistralai/mistral-vibe) | ✅ Підтримується | `capt mcp install vibe` |
...
**[HyperFrames][hyperframes]** — це відкрита, agent-native відеоплатформа від HeyGen, інтегрована як повноправний учасник в OpenDesign. Агент пише HTML + CSS + GSAP, а HyperFrames рендерить це в детермінований MP4 через headless Chrome + FFmpeg. Поєднайте її з **Seedance 2.0** для кінематографічного t2v / i2v, **Veo 3 / Sora 2 / Kling 2** для маршрутизованих варіантів моделей та **Suno v5 / Lyria 2** для аудіошару.
...
OpenDesign (OD) — це альтернатива з відкритим кодом. Той самий цикл, та сама ментальна модель «спершу артефакт», без жодної прив'язки:
...
- 🌍 **Композиційна на чотирьох площинах.** **Плагіни** несуть workflow · **функціональні навички** — поведінку агента · **шаблони дизайну** — креслення рендерингу · **дизайн-системи** — бренд.
...
Ви можете користуватися OpenDesign, жодного разу не відкриваючи GUI — викликайте його як навичку, плагін або сервер MCP усередині Claude Code, Codex, Cursor, Copilot, OpenClaw, Antigravity, Hermes, Kimi тощо.
...
У локальному CLI-запуску з файловою системою агент компонує функціональну навичку або шаблон із `DESIGN.md`, пише канонічні файли, а OpenDesign їх показує. BYOK/API без файлових інструментів повертає один повний блок `<artifact>`.
...
5. **OpenDesign стає розумнішим у міру використання.** Ваші скриншоти, шрифти, палітри та підтверджені артефакти накопичуються як значення за замовчуванням для наступної сесії. Менше переробки, менше дрейфу.

---

## Використання OpenDesign зі свого кодувального агента

OpenDesign постачає **сервер MCP через stdio** та **скрипти встановлення** для кожного агента. Будь-який MCP-сумісний агент в іншому репозиторії може читати файли з ваших локальних проєктів OpenDesign безпосередньо — CSS токенів, JSX-компоненти, вхідний HTML — як структурований API, до якого можна звертатися за іменем. Агент завжди бачить живий файл, а не застарілий експорт.
...
## Навички та шаблони дизайну

**100+ функціональних навичок містяться в [`skills/`](../../skills/)** і дають повторно використовувану поведінку, довідки чи утиліти. Рендерні starter-и відокремлено в [`design-templates/`](../../design-templates/) і вони потрапляють до каталогу шаблонів, не реєстру функціональних навичок.

Два **режими** є основою каталогу шаблонів: `prototype` і `deck`; інші шаблони покривають `image`, `video`, `audio` та utility.

| Шаблон дизайну | Режим | Сценарій | Що він створює |
|---|---|---|---|
...
Протокол і поділ каталогів → [`docs/skills-protocol.md`](../../docs/skills-protocol.md). `GET /api/skills` обслуговує навички, `GET /api/design-templates` — шаблони.

---

## Дизайн-системи

**151 пакет дизайн-систем із `DESIGN.md` у центрі** постачається з репозиторієм. Старі пакети можуть містити лише Markdown-контракт; нові також можуть містити `manifest.json`, скомпільований `tokens.css`, fixtures, assets і докази походження. Каталог змішує upstream-похідні та власні доповнення; [`design-systems/README.md`](../../design-systems/README.md) описує форму й походження.
...
| [`examples/`](../../plugins/_official/examples/) | 183 | Реміксувані еталонні результати |
...
Плагіни мають повний паритет між **вебінтерфейсом** та **CLI `capt`** — ті самі кінцеві точки `/api/plugins`, виберіть те, що підходить.
...
Плагіну OpenDesign потрібні `open-design.json` і payload його типу. Навички та сценарії також містять `SKILL.md`; інші типи використовують власний payload:
...
Основні поля: `specVersion`, `name`, `version`, опційний `compat.agentSkills[].path` лише при експорті Agent Skill, а також `od.kind`, `od.taskKind`, `od.mode`, `od.capabilities[]` і `od.inputs[]`.
...
1. Покладіть теку плагіна в [`plugins/community/`](../../plugins/community/) (сторонні плагіни), або — щоб постачати його в комплекті з OpenDesign — у відповідний рівень [`plugins/_official/`](../../plugins/_official/).
2. Пройдіть валідацію: `capt plugin validate`, `pnpm guard`, `pnpm --filter @open-design/plugin-runtime typecheck`.
...
- [x] Демон + 26 runtime-визначень на 25 різних CLI + реєстри навичок/шаблонів + каталог систем
- [x] Вебзастосунок + чат + форма запитань + селектор 5 напрямів + прогрес todo + попередній перегляд у пісочниці
- [x] 100+ функціональних навичок · окремий каталог шаблонів · 151 пакет дизайн-систем · 5 напрямів · 5 рамок
...
- [x] **0.9.0** — OpenDesign Cloud (офіційний Model Router, вбудований у застосунок: нуль конфігурації, вхід в один клік)
...
- [ ] `npx capt init` для каркасування проєкту з `DESIGN.md`
...
OpenDesign продовжує рухатися, бо контриб'ютори — дизайнери, інженери, автори промптів — продовжують приходити. Багато з найуживаніших навичок, дизайн-систем і плагінів написали люди поза основною командою.
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | Платформа моушн-графіки HTML→MP4, інтегрована як повноправний `hyperframes-html` в OpenDesign. |
