...
5. A third party can publish a skill repo; `capt skill add <url>` installs it and it works.
...
- Topology B: Vercel web + tunneled local daemon
  - Ship a helper subcommand: `capt daemon --expose` using `cloudflared` (opt-in, documented)

**Dev experience:**
- `capt skill test` with cheap-model runs
- Skill author starter template: `capt skill scaffold`
...
| SE2 | Automation template registry exposed in both web UI and `capt automation`. |
...
`capt memory tree list/view/edit/move` expose a derived editable tree while the
...
`capt automation proposal list/get/apply/reject` can review memory-node, skill,
...
`/api/automation-source-packets`, and `capt automation source ingest/list/get`.
...
| Skill security (malicious skill via `capt skill add`) | user machine compromise | install-time warning; rely on agent's own permission model; document best practices |
