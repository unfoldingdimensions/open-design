...
run `capt brand preview <brandId>` and the daemon re-renders the page (the page
...
layout posture, update `brand.json` and re-run `capt brand preview`. Partial data
...
```bash
capt brand finalize <brandId> --json
```
