...
function resolveToolsPackRoot(startDir: string): string {
  ...
  for (let depth = 0; depth < maxDepth; depth += 1) {
    try {
      ...
      if (parsed.name === "@capydesign/tools-pack") {
        return current;
      }
    } catch {
 ...
 * teaches it CapyDesign's versioned JSONL stdio protocol.
 */
export async function packBundledDshRuntime({
  ...
  if (packageJson.name !== "@capydesign/dsh-runtime" || typeof packageJson.version !== "string") {
    throw new Error("tools-pack: invalid @capydesign/dsh-runtime package metadata");
  }
