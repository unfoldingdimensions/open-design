...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "CapyDesign-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "CapyDesign-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "CapyDesign"),
    installedExePath: join(namespaceRoot, "runtime", "install", "CapyDesign", "CapyDesign.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "CapyDesign-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "CapyDesign-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "CapyDesign-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "CapyDesign", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "CapyDesign.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "CapyDesign.exe"))).resolves.toMatchObject({
