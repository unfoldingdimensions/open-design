...
async function writeHyperframesRuntimeFixture(options: {
  ...
  const bundledNodePath = options.platformName === "win32"
    ? join(appPath, "CapyDesign.exe")
    : join(appPath, "Contents", "MacOS", "CapyDesign");
...
async function writeRootWebPackage(resourcesRoot: string): Promise<void> {
  const webPackageRoot = join(resourcesRoot, "app", "node_modules", "@capydesign", "web");
  await mkdir(join(webPackageRoot, "dist", "sidecar"), { recursive: true });
  await writeFile(join(webPackageRoot, "package.json"), "{\"name\":\"@capydesign/web\"}\n", "utf8");
...
async function runFixture(options: {
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "CapyDesign.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "CapyDesign.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "CapyDesign Helper.app"), { recursive: true });
  }
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "CapyDesign" } },
    });
  } catch (error) {
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/CapyDesign Helper\.app$/),
          expect.stringMatching(/CapyDesign\.app$/),
        ]),
      );
