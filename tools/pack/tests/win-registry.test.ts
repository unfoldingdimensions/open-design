...
describe("stripRegistryQuotedValue", () => {
  it("returns the quoted body when the value is fully quoted", () => {
    expect(stripRegistryQuotedValue('"C:\\Program Files\\CapyDesign\\Uninstall.exe"')).toBe(
      "C:\\Program Files\\CapyDesign\\Uninstall.exe",
    );
  });

  it("returns the inner segment when the quoted region carries trailing flags", () => {
    expect(stripRegistryQuotedValue('"C:\\Program Files\\CapyDesign\\Uninstall.exe" /S')).toBe(
      "C:\\Program Files\\CapyDesign\\Uninstall.exe",
    );
  });

  it("trims unquoted values and leaves them otherwise unchanged", () => {
    expect(stripRegistryQuotedValue("  C:\\CapyDesign\\Uninstall.exe  ")).toBe(
      "C:\\CapyDesign\\Uninstall.exe",
    );
  });
