...
import { LAUNCHER_SCHEMA_VERSION } from "@capydesign/launcher-proto";
...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "CapyDesign-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "CapyDesign-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "CapyDesign Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "CapyDesign Beta", "CapyDesign.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "CapyDesign-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "CapyDesign-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "CapyDesign-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "CapyDesign Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "CapyDesign.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "CapyDesign.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "CapyDesign.exe"),
      source: "namespace",
...
describe("tools-pack Windows launcher payload archives", () => {
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/CapyDesign.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "CapyDesign.exe"));
