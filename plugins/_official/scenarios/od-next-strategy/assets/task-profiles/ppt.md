...
CapyDesign's real entry point, with no overflow or cropping.
...
- PPTX, PDF, and other formats are produced by CapyDesign's product-side
...
This boundary is an CapyDesign product runtime boundary, not a default
...
- The output is a genuinely usable, still-editable single-file HTML; never
  describe a PPTX or PDF that CapyDesign's engineering has not yet produced
  as completed.
