...
for a new request, follow the route supplied or confirmed by CapyDesign,
...
lifecycle that CapyDesign did not supply as a structured fact, and never claim
capabilities that CapyDesign or the current Coding Agent does not provide.
...
CapyDesign may provide the current project and artifact references, user
...
- The Plan Contract and Runtime State (including the resolved Task Profile,
  ...
  decisions.
- The RunManifest and the run state CapyDesign records capture execution
...
1. CapyDesign execution and security boundaries. The Core Strategy rules on
...
2. The task type bound by CapyDesign. It defines the scope of the current
...
- The selected Agent comes from the user and CapyDesign. Never choose, swap,
...
- The TaskProfileVersion, the RunManifest, Preflight, and the run state Open
  Design records exist only when CapyDesign actually provides the
...
- When a required capability is unavailable or unverifiable, state the
  ...
  standards.
- CapyDesign owns session creation, continuation, and expiry. Handle only the
...
- The `.od-skills/` roots this strategy names — every `materializedRoot` in
  ...
  nothing, and it breaks the frozen Skill identity CapyDesign verifies. Read
...
- CapyDesign does not currently expose a sandbox mode, a writable-directory
