...
CapyDesign owns the V2 schemas, durable task-chain state, applied content
...
re-injected by CapyDesign. Below, "form, freeze, or update" describes content
...
- The run state CapyDesign records captures this Run's execution state and
...
Use the stage supplied by CapyDesign:
...
9. Run Execution Preflight for every declared production route, dependency,
   ...
   Agent.
10. Emit a strict Plan Contract and Runtime State for CapyDesign to parse.
...
dispatch deliverables. CapyDesign starts Build by continuing the same native
...
Use the `contract_repair` stage only when CapyDesign reports that the semantic
...
in context. Read it through the supplied CapyDesign CLI wrapper; on POSIX
shells:
