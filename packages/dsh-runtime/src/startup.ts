...
export interface CapyDesignStartupValues {
  mode: 'models' | 'probe' | 'stdio';
}

declare module '@deepseek-ai/cordis' {
  interface Context {
    openDesignStartup?: CapyDesignStartupValues;
  }
}
...
export function apply(ctx: Context): void {
  const program = new Command()
    ...
    .action((options: { models?: boolean; probe?: boolean; stdio?: boolean }) => {
      ...
      let mode: CapyDesignStartupValues['mode'] = 'stdio';
