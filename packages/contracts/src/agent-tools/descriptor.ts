 ...
 * `capt` CLI form, so it is exempt from the UI/CLI dual-track law — but only by
 ...
 * `/api/*` route and its `capt` subcommand.
 */
export interface ApiToolDescriptor extends AgentToolDescriptorBase {
  ...
  /** The equivalent `capt` CLI form, satisfying the dual-track law. */
