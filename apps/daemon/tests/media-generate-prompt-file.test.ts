// Gap-fill spec — `capt media generate` must accept --prompt-file <path|->.
//
...
//
// Every other prompt-taking `capt` action (run redesign, brand create, automation
// create, files version-create) already routes through readPromptFromFlags,
// which supports --prompt / --prompt-file / stdin. `capt media generate` was the
...
async function runCli(args: string[], input?: string): Promise<void> {
  ...
  // alone.
  expect(code, `capt media generate exited ${code}; stderr:\n${stderr}`).toBe(0);
}

describe('capt media generate --prompt-file', () => {
