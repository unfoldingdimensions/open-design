...
export async function startBrandExtraction(
  ...
  try {
    ...
    // first paint (below) or filled in by the agent's `capt brand preview` passes.
    //
...
async function finalizeBrandCore(opts: FinalizeBrandCoreOptions): Promise<BrandFinalizeResponse> {
  ...
  // `capt brand finalize`) drove the finalize. Best-effort — a reconcile failure
 ...
 * `POST /api/brands/:id/preview` (`capt brand preview`).
 */
...
const SEED_AUTHORING_GUIDANCE =
  'Persist engine-level overrides such as control height in `brand.json.seed` (for example, `{ "controlHeight": 44 }`). Do not edit `system/seed.json` or other generated `system/` files directly; `capt brand finalize` replaces them.';
...
function brandExtractionPrompt(input: {
  ...
  if (input.hasDesignMdSource && !input.hasWebsiteSource) {
    return [
      ...
      '',
      'Do not create a duplicate design system. Keep the same registered user design-system id. Update `brand.json` and `BRAND.md` incrementally, run `capt brand preview ' + input.brandId + '` after field groups, then run `capt brand finalize ' + input.brandId + '` when ready.',
...
function brandExtractionFallbackPrompt(input: {
  ...
  if (input.hasDesignMdSource && !input.hasWebsiteSource) {
    return [
      ...
      '',
      'Read `context/input-DESIGN.md`, then update `brand.json`, `BRAND.md`, and `DESIGN.md` progressively. Run `capt brand preview ' + input.brandId + '` after meaningful field groups, then `capt brand finalize ' + input.brandId + '` to register or update the same design system in place.',
      SEED_AUTHORING_GUIDANCE,
      '',
  ...
  return [
    ...
    '',
    'Write `brand.json` as soon as you have the name, a couple of measured colors, and a logo candidate, then run `capt brand preview ' + input.brandId + '` so the scaffold fills in progressively. Keep updating `brand.json`, `BRAND.md`, saved `logos/`, fonts, and `imagery/` samples as you measure each field group.',
    SEED_AUTHORING_GUIDANCE,
    '',
    'When the kit is complete and validates, run `capt brand finalize ' + input.brandId + '` (add `--json` for machine output). Fix validation errors and re-run finalize until the brand is registered and the design-system assets are ready.',
    '',
 ...
 * `capt brand finalize` after fixing a validation error or enriching the kit, and
