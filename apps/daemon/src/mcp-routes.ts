...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  function computeInstallPayload(): McpInstallPayload {
    ...
    // knows where the browser-facing CapyDesign studio is running.
  ...
  // External MCP server configuration. CapyDesign connects to these as a
...
function renderOAuthResultPage(opts: any) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to CapyDesign.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — CapyDesign</title>
