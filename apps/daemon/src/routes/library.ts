...
export function registerLibraryRoutes(app: Express, ctx: RegisterLibraryRoutesDeps): void {
  ...
  // `capt library sync`) bypasses the throttle.
  ...
  app.post('/api/library/ingest', async (req, res) => {
    ...
    // Manual uploads (local web UI / `capt library import`) are restricted to a
  ...
  // Force a full reconcile pass (the web "Sync" button + `capt library sync`).
