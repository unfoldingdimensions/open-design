...
//
//   - `capt`            → SKILL.md + open-design.json (canonical OD shape).
...
// even after an `capt plugin update` rotates the live source.
...
export async function exportPlugin(input: ExportInput): Promise<ExportResult> {
  ...
  const readme = [
    ...
    '',
    'This folder was produced by `capt plugin export`.',
    '',
  ].join('\n');
