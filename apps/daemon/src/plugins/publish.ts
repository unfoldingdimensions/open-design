// Phase 4 / spec §14.1 — `capt plugin publish --to <catalog>` PR-template launcher.
//
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      const bodyWithRegistry = [
        ...
        '- Required checks: `capt plugin validate`, `capt plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive CapyDesign metadata).');
  lines.push('');
  lines.push('Generated by `capt plugin publish` — see https://open-design.ai/docs/plugins-spec.md.');
  return lines.join('\n');
}
