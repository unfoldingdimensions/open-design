...
// DoctorReport shape the post-install `capt plugin doctor <id>`
// command emits.
//
// The lift from `capt plugin doctor`:
...
//
// Rationale: spec §16 Phase 4 ships `capt plugin scaffold`, `capt
// plugin export`, `capt plugin publish` for the author tooling slice.
// `capt plugin validate` closes the loop: the author can run lint
