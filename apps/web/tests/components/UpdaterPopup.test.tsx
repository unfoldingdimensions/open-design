...
import type { CapyDesignHostUpdaterStatusListener, CapyDesignHostUpdaterStatusSnapshot } from '@open-design/host';
import { installMockCapyDesignHost } from '@open-design/host/testing';

import { UpdaterPopup } from '../../src/components/UpdaterPopup';
import { I18nProvider } from '../../src/i18n';

function idleStatus(): CapyDesignHostUpdaterStatusSnapshot {
  return {
    arch: 'arm64',
    capabilities: {
...
describe('UpdaterPopup', () => {
  ...
  it('shows only the ready indicator until the user opens the install prompt', async () => {
    ...
    expect(screen.getByText('CapyDesign 1.2.3-beta.4 is ready. CapyDesign will close and open the installer.')).toBeTruthy();
  ...
  it('uses reinstall copy and the learn-more link for forced installer reinstalls', async () => {
    ...
    expect(
      screen.getByText('CapyDesign 1.2.3-beta.4 requires a full reinstall. CapyDesign will close and open the installer.'),
    ).toBeTruthy();
  ...
  it('uses localized ready prompt copy from the app i18n provider', async () => {
    ...
    expect(screen.getByText('CapyDesign 1.2.3-beta.4 已就绪。CapyDesign 会关闭并打开安装器。')).toBeTruthy();
  });
  ...
  it('uses install-and-restart copy for payload updates', async () => {
    ...
    expect(screen.getByText('CapyDesign 1.2.3-beta.4 已就绪。CapyDesign 会关闭并自动重启。')).toBeTruthy();
  });
  ...
  it('keeps the prompt in handoff loading after opening the installer', async () => {
    ...
    let resolveInstall: (status: CapyDesignHostUpdaterStatusSnapshot) => void = () => undefined;
    const install = vi.fn(() => new Promise<CapyDesignHostUpdaterStatusSnapshot>((resolve) => {
  ...
  it('recovers the handoff prompt if the app has not closed after the watchdog', async () => {
    ...
    try {
      ...
      expect(screen.getByTestId('updater-install-button').textContent).toBe('Quit CapyDesign');
  ...
  it('reacts to updater subscription events by showing the ready indicator only', async () => {
    const listeners = new Set<CapyDesignHostUpdaterStatusListener>();
    restoreHost = installMockCapyDesignHost({
      host: {
        updater: {
