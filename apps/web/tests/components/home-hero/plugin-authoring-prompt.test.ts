...
// summary turn freeform-recommended `capt plugin publish --to open-design`
...
describe('PLUGIN_AUTHORING_PROMPT_TEMPLATE', () => {
  ...
  it('forbids placeholder plugin.repo owners in generated manifests', () => {
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('capt plugin publish-repo generated-plugin --owner <github-login-or-org>');
  ...
  it('still drives the local validation chain', () => {
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('capt plugin validate');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('capt plugin pack');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('capt plugin install --source');
  });

  it('bans freeform publish / repo CLI suggestions in the summary turn', () => {
    // The agent transcript in #2332 had the agent recommending
    // `capt plugin publish --to open-design`, `gh repo create
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('capt plugin publish --to open-design');
  ...
  it('points the user at the plugin-folder card buttons instead', () => {
    ...
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toContain('CapyDesign PR');
    expect(PLUGIN_AUTHORING_PROMPT_TEMPLATE).toMatch(
