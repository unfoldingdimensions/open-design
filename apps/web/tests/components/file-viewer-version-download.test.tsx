...
const {
  ...
  isCapyDesignHostAvailableMock: vi.fn(() => false),
...
vi.mock('../../src/runtime/exports', async () => {
  ...
  return {
    ...
    isCapyDesignHostAvailable: isCapyDesignHostAvailableMock,
...
describe('FileViewer version download actions', () => {
  afterEach(() => {
    cleanup();
    vi.clearAllMocks();
    isCapyDesignHostAvailableMock.mockReturnValue(false);
    vi.unstubAllGlobals();
  });

  it('routes current version PDFs through the main download PDF exporter', async () => {
    isCapyDesignHostAvailableMock.mockReturnValue(true);
