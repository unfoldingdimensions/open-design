...
interface Props {
  ...
  // Click handler for the post-completion "Share to CapyDesign" submission
  // action. ProjectView wires this to handleSend with the bundled
  // `od-share-to-community` trigger prompt.
  onShareToCapyDesign?: () => void;
  shareToCapyDesignBusy?: boolean;
  ...
  */
  ...
  const continueRemaining =
    isLast && onContinueRemainingTasks && continuableTodos.length > 0
      ...
      : undefined;
  const canShowCapyDesignSubmission = !!onShareToCapyDesign && showFeedback && runSucceeded;
  const showCapyDesignSubmission =
    canShowCapyDesignSubmission && (!!isLast || shareToCapyDesignBusy);
        ...
        */}
        ...
        {showNextStepActions ? (
          <NextStepActions
            ...
            onShareToCapyDesign={showCapyDesignSubmission ? onShareToCapyDesign : undefined}
            shareToCapyDesignBusy={shareToCapyDesignBusy}
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|CapyDesign PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
