// Content fixtures for the "Use CapyDesign everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'CapyDesign works wherever your agent works',
    intro:
      'CapyDesign is more than a window — it is a local privileged daemon ' +
      "(`capt`) plus a Skills + Design-Systems + Atoms registry. Once it's " +
    ...
    bullets: [
      'CLI — `capt <command>` for headless scripts, CI, and shell automation.',
      'MCP server — wires CapyDesign as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without CapyDesign at all.',
    ...
    snippets: [
      {
        ...
        language: 'bash',
        body: 'pnpm tools-dev\n# or, if `capt` is on your PATH (packaged install):\ncapt --port 7456',
      },
      {
      ...
      },
      {
        ...
        body:
          ...
          '# Then start CapyDesign in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
  ...
  },
  {
    id: 'cli',
    tabLabel: 'CLI · capt',
    heading: 'Drive CapyDesign from any shell',
    intro:
      'The `capt` bin ships with the daemon and is the same binary used by ' +
    ...
    bullets: [
      '`capt` (no args) — boots the daemon and opens the web UI.',
      '`capt media generate ...` — produce image / video / audio bytes through the unified media protocol.',
      '`capt project create` + `capt run start` — create a project, send a message, and stream the run.',
      '`capt plugin install <source>` / `capt plugin apply <id>` — install and apply community plugins.',
      '`capt skills list` / `capt design-systems list` — inspect what is available locally.',
      '`capt status` / `capt doctor` — verify daemon health and detect agent CLIs on your PATH.',
    ],
    snippets: [
      {
      ...
      },
      {
        ...
        body:
          ...
          'PROJECT_JSON=$(capt project create \\\n' +
          '  --name "Investor pitch" \\\n' +
      ...
      },
      {
        ...
        language: 'bash',
        body:
          'capt files list "$PROJECT_ID" --daemon-url "$DAEMON_URL" --json\n' +
          'capt files read "$PROJECT_ID" index.html --daemon-url "$DAEMON_URL" | head',
      },
      {
        label: 'Inventory locally available skills and design systems',
        language: 'bash',
        body: 'capt skills list --json\ncapt design-systems list --json',
      },
      {
        label: 'Check seeded artifacts through the CLI',
        language: 'bash',
        body:
          'capt project list --daemon-url http://127.0.0.1:7456\n' +
          'capt files list <seed-project-id> --daemon-url http://127.0.0.1:7456\n' +
          'capt files read <seed-project-id> index.html --daemon-url http://127.0.0.1:7456 | head',
      },
      {
    ...
    footer:
      ...
      'second instance for tests. From a source checkout, replace `capt` with ' +
      '`"$OD_NODE_BIN" "$OD_BIN"` after exporting those variables.',
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose CapyDesign as an MCP server to any coding agent',
    intro:
      'CapyDesign ships with a Model Context Protocol server (`capt mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover CapyDesign tools ' +
    ...
    bullets: [
      'Stdio transport — no extra port, the client spawns `capt mcp` directly.',
    ...
    snippets: [
      ...
      },
      {
        ...
        language: 'bash',
        body: 'capt mcp live-artifacts',
      },
    ],
    footer:
      'In the CapyDesign app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    heading: 'Drop-in Skills for any agent — even without CapyDesign running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). CapyDesign extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside CapyDesign and by a vanilla ' +
    ...
    bullets: [
      ...
      'Headless: an agent with `capt` on its PATH can call `capt skills list` then run any skill; the daemon is optional for read-only flows.',
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + CapyDesign extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        language: 'bash',
        body: 'capt skills list --json | jq \'.skills[].name\'',
      },
      {
        ...
        body:
          ...
          '# Shell 1: start CapyDesign after ingesting.\n' +
          'pnpm tools-dev\n' +
          '# Shell 2: inspect the produced projects.\n' +
          'capt project list --json --daemon-url http://127.0.0.1:7456',
      },
    ],
    footer:
