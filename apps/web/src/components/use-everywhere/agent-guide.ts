...
// everything it needs to install CapyDesign, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional CapyDesign version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# CapyDesign — agent setup guide');
  lines.push('');
  lines.push(
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported CapyDesign version: \`${options.versionHint}\``);
  }
  lines.push('');
