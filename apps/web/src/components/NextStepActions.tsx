...
interface Props {
  ...
  skills?: SkillSummary[];
  // Contribute the artifact to the CapyDesign community gallery.
...
export function NextStepActions({
  ...
  const handleContribute = useCallback(() => {
    if (!onShareToCapyDesign || shareToCapyDesignBusy) return;
    track('share_to_open_design');
    onShareToCapyDesign();
    closeAll();
  }, [closeAll, onShareToCapyDesign, shareToCapyDesignBusy, track]);
  ...
  const canContribute = !!onShareToCapyDesign;
        ...
        ? createPortal(
            ...
            >
              ...
              {canContribute ? (
                ...
                >
                  <Icon
                    name={shareToCapyDesignBusy ? 'spinner' : 'globe'}
                    size={14}
                    className={shareToCapyDesignBusy ? 'icon-spin' : styles.toolboxRowIcon}
                  />
