// Trigger prompt for the post-completion "Share to CapyDesign" submission
...
export const SHARE_TO_COMMUNITY_PROMPT = [
  'Package the work I just finished in this project as an CapyDesign plugin so I can install it into My plugins and open a PR to the CapyDesign community catalog.',
  '',
  ...
  '',
  'Validate the plugin locally before reporting: run `capt plugin validate` on the folder, then `capt plugin pack` for a tarball, then `capt plugin install --source <absolute-folder-path>` to confirm the install path works.',
  '',
  'When the work above is done, write a single summary turn covering: files created, `capt plugin validate` status, local install / run status, and `capt plugin pack` output. Then STOP.',
  '',
