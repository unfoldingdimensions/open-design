...
- Consume `@capydesign/sidecar-proto`, `@capydesign/sidecar`, and `@capydesign/platform` primitives; do not hand-build stamp flags or process matching logic.
...
- `OD_RESOURCE_ROOT` is for daemon non-Next read-only resources. Keep its bundled-tree contract aligned with the authoritative list in `tools/pack/src/resources/index.ts` instead of maintaining a shorter copy here.
