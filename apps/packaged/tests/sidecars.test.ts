...
describe('resolvePackagedElectronNodeCommand', () => {
  it('uses the hidden Electron helper as the macOS Electron-as-Node command when available', async () => {
    ...
    try {
      const appPath = posix.join(root.replaceAll('\\', '/'), 'CapyDesign.app');
      const execPath = posix.join(appPath, 'Contents', 'MacOS', 'CapyDesign');
      const helperPath = posix.join(
        appPath,
        'Contents',
        'Frameworks',
        'CapyDesign Helper.app',
        'Contents',
        'MacOS',
        'CapyDesign Helper',
      );
  ...
  it('falls back to the main executable when the macOS helper is unavailable', async () => {
    ...
    try {
      const execPath = join(root, 'CapyDesign.app', 'Contents', 'MacOS', 'CapyDesign');
  ...
  it('keeps the main executable on non-macOS platforms', async () => {
    const execPath = '/opt/CapyDesign/open-design';
...
describe('buildPackagedDaemonSpawnEnv', () => {
  ...
  it('forwards the signed packaged launcher used to bootstrap MCP headlessly', () => {
    const env = buildPackagedDaemonSpawnEnv(fakePaths(), {
      appVersion: '1.2.3',
      daemonCliEntry: '/Applications/CapyDesign.app/Contents/Resources/app/prebundled/daemon/daemon-cli.mjs',
      legacyDataDir: null,
      mcpBootstrapArgs: [
        '-g',
        '-j',
  ...
  it('forwards the packaged node command as OD_NODE_BIN for agent wrapper calls', () => {
    const env = buildPackagedDaemonSpawnEnv(fakePaths(), {
      ...
      daemonCliEntry: null,
      legacyDataDir: null,
      nodeCommand: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\CapyDesign\\resources\\open-design\\bin\\node.exe',
      requireDesktopAuth: true,
    });

    expect(env.OD_NODE_BIN).toBe(
      'C:\\Users\\Ada\\AppData\\Local\\Programs\\CapyDesign\\resources\\open-design\\bin\\node.exe',
    );
  });
