...
async function createMacInstalledOuter(): Promise<{
  ...
  if (process.platform === "win32") {
    const launchPath = "/Applications/CapyDesign.app";
    const executablePath = `${launchPath}/Contents/MacOS/CapyDesign`;
    return {
      executablePath,
  ...
  roots.push(root);
  const launchPath = join(root, "CapyDesign.app");
  const executablePath = join(launchPath, "Contents", "MacOS", "CapyDesign");
...
async function createWindowsInstalledOuter(
  ...
  if (process.platform !== "win32") {
    const executablePath = `C:\\Program Files\\CapyDesign\\${executableName}`;
...
describe("createObsoleteInstalledOuterRetirement", () => {
  it("stops only the exact installed outer root and its descendants", async () => {
    ...
    const snapshots = [
      ...
      snapshot(102, 101, `${launchPath}/Contents/Frameworks/CapyDesign Helper.app/Contents/MacOS/CapyDesign Helper`),
      snapshot(103, 102, "helper-child"),
      snapshot(104, 1, `${executablePath} Helper`),
      snapshot(105, 1, "/unrelated/CapyDesign"),
      snapshot(900, 1, "/payload/CapyDesign.app/Contents/MacOS/CapyDesign"),
    ];
    const stopProcesses = stopMock();
    const logger = { info: vi.fn(), warn: vi.fn() };
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/CapyDesign.app/Contents/MacOS/CapyDesign",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/CapyDesign.app/Contents/MacOS/CapyDesign",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("does nothing outside an exact payload desktop process", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/installed/CapyDesign",
      currentPid: 900,
      installedLaunchPath: launchPath,
  ...
  it("rejects a symlinked install executable before enumerating processes", async () => {
    ...
    if (process.platform === "win32") {
      launchPath = "/Applications/CapyDesign.app";
      const executablePath = `${launchPath}/Contents/MacOS/CapyDesign`;
      ...
      roots.push(root);
      launchPath = join(root, "CapyDesign.app");
      ...
      await symlink(target, join(executableDirectory, "CapyDesign"));
    }
    const listProcessSnapshots = vi.fn(async () => []);
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/CapyDesign",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger: { info: vi.fn(), warn: vi.fn() },
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/CapyDesign",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("refuses to stop an outer tree that contains the current payload", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      ...
      listProcessSnapshots: async () => [
        ...
        snapshot(900, 800, "/payload/CapyDesign"),
...
describe("Windows obsolete installed outer retirement", () => {
  it("stops the revalidated exact installed outer root and its current descendants", async () => {
    ...
    const firstSnapshots = [
      ...
      snapshot(202, 201, "CapyDesign.exe --type=gpu-process"),
      snapshot(203, 202, "CapyDesign.exe --type=utility"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\CapyDesign.exe"),
    ];
    const secondSnapshots = [
      snapshot(201, 1, `"${executablePath}"`),
      snapshot(202, 201, "CapyDesign.exe --type=gpu-process"),
      snapshot(206, 202, "CapyDesign.exe --type=renderer"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\CapyDesign.exe"),
    ];
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      ...
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "c:\\payload\\CapyDesign.exe",
      platform: "win32",
    }, { inspectInstalledOuterPath, listProcessSnapshots, stopProcesses });
  ...
  it("skips a reused root pid when the second snapshot no longer matches the executable", async () => {
    ...
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\CapyDesign.exe",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger: { info: vi.fn(), warn: vi.fn() },
      payloadDesktopProcess: true,
  ...
  it("rejects installed executables whose basename does not match the payload", async () => {
    const { inspectInstalledOuterPath, launchPath } = await createWindowsInstalledOuter("Not CapyDesign.exe");
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\CapyDesign.exe",
  ...
  it("refuses a revalidated outer tree that contains the current payload", async () => {
    ...
    const snapshots = [
      ...
      snapshot(900, 800, "C:\\payload\\CapyDesign.exe"),
    ];
  ...
  it("rejects a symlinked Windows executable before enumerating processes", async () => {
    const executablePath = "C:\\Program Files\\CapyDesign\\CapyDesign.exe";
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\CapyDesign.exe",
