...
describe("desktop updater feed", () => {
  it("resolves the installed outer version from the platform bundle layout", async () => {
    const root = makeRoot();
    try {
      const appRoot = join(root, "CapyDesign.app");
      ...
      const winExe = join(root, "win-install", "CapyDesign Beta.exe");
      ...
      const malformedExe = join(root, "broken-install", "CapyDesign.exe");
