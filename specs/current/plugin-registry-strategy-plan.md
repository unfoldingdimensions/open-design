...
- `capt` remains the canonical headless API for CRUD, search, install, update, trust, pack, doctor, and publish.
...
- `apps/daemon/src/cli.ts` already exposes `capt marketplace add/list/info/search/refresh/remove/trust`.
...
Registry backend
  ...
  Later: DatabaseRegistryBackend with the same contract.
        |
        | capt marketplace search/install/upgrade
        v
Installed plugin
...
- **`capt` CLI** remains the canonical client. Every UI action must map to a CLI operation or daemon API that the CLI can also drive.
...
Create plugin
  ...
  -> capt plugin publish opens GitHub registry PR
...
- The source-of-truth repo is declared in `open-design.json` as `plugin.repo`. Published plugins can be "anything that packs": no special repo layout is required beyond passing `capt plugin validate` and `capt plugin pack`, shipping a runnable skill anchor, and declaring `plugin.repo`.
...
- `capt marketplace add <enterprise-url>` and in-app Sources continue to work; only the source backend changes.
...
- CLI commands must not expose GitHub-specific nouns except where the source explicitly is GitHub. `capt plugin publish --to open-design` may use `gh` internally, but the command contract should survive a later database backend.
...
- Installing the `capt` CLI should ensure `gh` is available. If the host has no `gh`, the installer bootstraps it using the platform package path available to that distribution channel, or fails with a precise remediation when auto-install is impossible.
- `capt plugin login` wraps `gh auth login` with OpenDesign copy and required scopes/host guidance.
- `capt plugin whoami` wraps `gh auth status` plus `gh api user` and prints the active account, host, scopes, and whether it can publish to the configured registry repo.
- `capt plugin logout` can wrap `gh auth logout` only after an explicit confirmation, because it affects the user's global GitHub CLI session.
...
Keep existing `capt marketplace` and `capt plugin` naming. Avoid adding a second `registry` noun until there is a real non-marketplace backend exposed to users.

### Marketplace Source Commands

- [ ] `capt marketplace add <url> [--trust official|trusted|restricted] [--github-host <host>] [--refresh daily|manual|startup]`
- [x] `capt marketplace list [--json]`
- [x] `capt marketplace info <id> [--json]`
- [ ] `capt marketplace plugins <id> [--query <q>] [--tag <tag>] [--json]`
- [x] `capt marketplace search "<query>" [--tag <tag>] [--json]`
- [x] `capt marketplace refresh <id>`
- [x] `capt marketplace remove <id>`
- [x] `capt marketplace trust <id> --trust official|trusted|restricted`
- [ ] `capt marketplace login <id>` (delegates to `gh auth login` for the marketplace host)
- [ ] `capt marketplace whoami <id>` (delegates to `gh auth status` / `gh api user`)
- [ ] `capt marketplace doctor [id] [--strict] [--json]`

### Plugin Consumer Commands

- [x] `capt plugin install <source-or-name>`
- [ ] `capt plugin install <name>@<version-or-tag> [--from <marketplace-id>] [--save-lock]`
- [ ] `capt plugin upgrade [id] [--policy latest|pinned|range] [--dry-run]`
- [ ] `capt plugin outdated [--json]`
- [x] `capt plugin list/info/search/apply/run/trust/uninstall/doctor`
- [ ] `capt plugin view <name> [--from <marketplace-id>] [--versions] [--json]`
- [ ] `capt plugin lock write|verify|diff`

### Plugin Author Commands

- [x] `capt plugin login [--host <github-host>]`
- [x] `capt plugin whoami [--host <github-host>] [--json]`
- [x] `capt plugin scaffold`
- [x] `capt plugin validate`
- [x] `capt plugin pack`
- [ ] `capt plugin publish --to open-design [--dry-run]`
- [ ] `capt plugin publish --to marketplace-json --catalog <path-or-url> [--branch <name>]`
- [ ] `capt plugin publish --to github --repo <owner/repo> [--public|--private]`
- [ ] `capt plugin deprecate <name>@<version> --reason <text>`
- [ ] `capt plugin yank <name>@<version> --reason <text>`

Publish v1 should use `gh`:

- `capt plugin login` wraps `gh auth login`; `capt plugin whoami` wraps `gh auth status` and `gh api user`.
- `capt plugin publish --to open-design` validates, packs if needed, renders the registry entry, creates a branch, opens a PR, and prints the PR URL.
- `capt plugin publish` uses `gh api`, `gh repo fork`, and `gh pr create` through a testable `GhClient`; it does not implement a parallel GitHub OAuth flow.
...
- Show install command, `capt plugin view`, `capt plugin run` example, version selector, dist-tags, changelog, source repo, publisher, license, integrity, capabilities, connector requirements, examples, and security notes.
...
- "Self-host your own source" flow with static JSON instructions and `capt marketplace add`.
...
- `capt marketplace add` stores URL, trust tier, GitHub host, refresh policy, and cache policy.
...
- [x] Add `capt marketplace plugins <id>` with pagination/search/filter.
- [x] Add `capt plugin install <name>@<version-or-tag>`.
...
- [ ] Add `capt plugin lock verify`.
- [ ] Add `capt plugin outdated`.
...
- [ ] Make `gh` an explicit `capt` CLI dependency and installer prerequisite/bootstrap step.
- [x] Add `capt plugin login` and `capt plugin whoami` as wrappers over `gh auth login/status` and `gh api user`.
- [ ] Add `capt plugin publish --to open-design --dry-run --json`.
...
- [ ] Run `capt plugin validate`, `pack`, `doctor`, and integrity calculation before PR creation.
...
- [x] Add `capt plugin publish --to marketplace-json --catalog <path>` for self-hosted static catalogs.
...
- [x] Add install/use/upgrade card states for available entries. Current install uses the existing bare-name `capt plugin install <name>` path and now preserves provenance; explicit `--from <marketplace-id>` remains a P1 follow-up.
...
- [x] Make `Create plugin` launch an agent-assisted authoring flow backed by `capt plugin scaffold/validate/pack/publish`, including local install/run validation before publish and `gh` login/whoami checks before opening a registry PR. Current slice updates the agent prompt and CLI wrapper; full GitHub PR mutation remains in P2.
...
- [x] Add private GitHub/GitHub Enterprise marketplace auth entry through `capt marketplace login <id|url> --host <host>`, delegated to `gh`.
...
- [x] Add `capt marketplace doctor` checks: entry naming/source/yank/capability/license/integrity basics, with strict mode for warnings.
...
3. **Available marketplace API**
   - Files: daemon `/api/marketplaces/:id/plugins`, CLI `capt marketplace plugins`, contracts response shape.
...
6. **GitHub CLI auth wrapper**
   ...
   - Outcome: `capt plugin login/whoami` provide the product auth surface while reusing the user's `gh` session.
...
- Should `capt plugin publish --to marketplace-json` mutate a local catalog file directly or create a branch/PR when the catalog URL maps to GitHub? Recommendation: support both, but default to PR when a GitHub remote is detectable.
...
- Source repo shape: accept anything that passes `capt plugin validate` and `capt plugin pack`.
