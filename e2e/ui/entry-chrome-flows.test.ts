...
test('[P1] cold Home defers Automations reads until the route becomes active', async ({ page }) => {
  ...
  try {
    ...
    await page.getByText('Loading CapyDesign…').waitFor({ state: 'hidden', timeout: T.long });
...
test('[P1] onboarding lands on the home composer without a recommended-start strip', async ({ page }) => {
  ...
  await page.getByRole('radio', { name: /CapyDesign Hosted/i }).click();
...
test('[P1] Use everywhere guide uses daemon MCP install info and copies an agent guide', async ({ page }) => {
  ...
  await page.route('**/api/mcp/install-info', async (route) => {
    await route.fulfill({
      json: {
        command: '/Applications/CapyDesign.app/Contents/MacOS/od',
  ...
  await expect(mcpSection).toContainText('/Applications/CapyDesign.app/Contents/MacOS/od');
  ...
  await expect
    ...
    .toContain('/Applications/CapyDesign.app/Contents/MacOS/od');
  await expect
    .poll(() => page.evaluate(() => (window as unknown as { __copiedTexts?: string[] }).__copiedTexts?.at(-1) ?? ''))
...
async function gotoEntryHome(page: Page) {
  await page.goto('/', { waitUntil: 'domcontentloaded' });
  await page.getByText('Loading CapyDesign…').waitFor({ state: 'hidden', timeout: T.long });
  const privacyDialog = page.getByRole('dialog').filter({ hasText: 'Help us improve CapyDesign' });
