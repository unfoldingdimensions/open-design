...
test('[P0] UI-created Personal project recovers preview and write authority after reload', async ({ page }) => {
  ...
  try {
    ...
    // (`Loading CapyDesign…`) and the project-route workspace-context gate
...
async function expectWorkspaceReady(page: Page) {
  await expect(page).toHaveURL(/\/projects\//);
  await page.getByText('Loading CapyDesign…').waitFor({ state: 'hidden', timeout: T.long }).catch(() => {});
...
async function dismissPrivacyDialog(page: Page) {
  const privacyRegion = page.getByRole('region', { name: /Help us improve CapyDesign/i });
