...
test('[P0] after local Sign out, the app returns to Cloud sign-in without clearing setup', async ({ page }) => {
  ...
  // "CapyDesign MCP" nav item, so a surface-wide /CapyDesign/i now resolves
  ...
  await expect(page.getByRole('button', { name: /Sign in to CapyDesign|登录 CapyDesign/i })).toBeVisible();
