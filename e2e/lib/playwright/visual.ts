...
const VISUAL_PLUGINS = [
  ...
  makeVisualPlugin({
    ...
    description: 'Migrate a Figma frame into an editable CapyDesign project.',
    mode: 'prototype',
...
export async function waitForVisualReady(page: Page): Promise<void> {
  await page.getByText('Loading CapyDesign…').waitFor({ state: 'hidden', timeout: T.xlong });
...
export async function prepareVisualAvatarMenu(page: Page): Promise<Locator> {
  ...
  // The composer popover is a model picker: the CapyDesign account card is
  // conditional (CapyDesign has to be installed), so gate on the model list.
