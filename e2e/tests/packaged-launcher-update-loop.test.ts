...
type PlatformCase = {
  ...
  productName: "CapyDesign" | "CapyDesign Beta" | "CapyDesign Prerelease";
...
const platformCases: PlatformCase[] = [
  {
    ...
    expectedPayloadExecutablePath: (root, namespace) =>
      join(root, "launcher", "channels", "beta", "namespaces", namespace, "versions", "1.2.3-beta.5", "payload", "CapyDesign.exe"),
  ...
  },
  {
    ...
    currentVersion: "1.2.3-beta.4",
    expectedPayloadExecutablePath: (root, namespace) =>
      join(root, "launcher", "channels", "beta", "namespaces", namespace, "versions", "1.2.3-beta.5", "payload", "CapyDesign Beta.app", "Contents", "MacOS", "CapyDesign Beta"),
    expectedResourceRoot: (root, namespace) =>
      join(root, "launcher", "channels", "beta", "namespaces", namespace, "versions", "1.2.3-beta.5", "payload", "CapyDesign Beta.app", "Contents", "Resources", "open-design"),
  ...
  },
  {
    ...
    expectedPayloadExecutablePath: (root, namespace) =>
      join(root, "launcher", "channels", "prerelease", "namespaces", namespace, "versions", "1.2.3-prerelease.5", "payload", "CapyDesign Prerelease.app", "Contents", "MacOS", "CapyDesign Prerelease"),
    expectedResourceRoot: (root, namespace) =>
      join(root, "launcher", "channels", "prerelease", "namespaces", namespace, "versions", "1.2.3-prerelease.5", "payload", "CapyDesign Prerelease.app", "Contents", "Resources", "open-design"),
    fixturePlatformKey: "mac",
...
async function writeExtractedFloorMacPayload(destinationRoot: string): Promise<void> {
  const appBundleName = "CapyDesign.app";
  ...
  await writeFile(join(bundleRoot, "Contents", "MacOS", "CapyDesign"), "");
  await writeFile(join(resourcesRoot, "open-design", "bin", "node"), "");
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      ...
      entry: {
        cwd: `payload/${appBundleName}`,
        executable: `payload/${appBundleName}/Contents/MacOS/CapyDesign`,
      },
      namespace: "default",
      payloadRoot: "payload",
      platform: "darwin",
...
async function writeExtractedFloorWindowsPayload(destinationRoot: string): Promise<void> {
  const executableName = "CapyDesign.exe";
...
const floorPlatformTargets = {
  mac: {
    ...
    installedLaunchPath: (installedRoot) => join(installedRoot, "CapyDesign.app"),
  ...
  win: {
    ...
    installedLaunchPath: (installedRoot) => join(installedRoot, "CapyDesign.exe"),
